...
module.exports = class Subscriptions {
			...
			.mergeMap(({
				...
				return Observable.pairs(subscriptions)
					.mergeMap(([
						...
						return executor(root, extendContext)
							.map(({
								args,
								context,
								operationName,
	...
	run(namespace, event, root = {}, extendContext = {}) {
		this.inbound.next({
			event,
			namespace,
			root,
	...
	subscribe(namespace, subscriber, requestString, variables = {}, context = {}) {
		...
		const hash = md5(`${requestString}${JSON.stringify(args)}${JSON.stringify(context)}`);
